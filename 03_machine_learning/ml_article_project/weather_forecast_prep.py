...
class MissingDataProcessor:
    def __init__(self, filepath):
        ...
        self.target = self.original_df['Rain'].to_frame()
        self.data = self.original_df.drop(["Rain"], axis='columns')
    ...
    def evaluate_imputation_methods(self, original_data, imputed_data):
        ...
        for column in original_data.columns:
            ...
            if mask.sum() > 0:
                ...
                imputed_subset = imputed_data.loc[mask.index][column]
    ...
    def process_and_impute(self, missing_percentages, methods):
        ...
        target_encoded['Rain'] = le.fit_transform(target_encoded['Rain'])
        ...
        for missing_percentage in missing_percentages:
            ...
            for method in methods:
                ...
                balanced_target.reset_index(drop=True, inplace=True)
                
                imputation_metrics = self.evaluate_imputation_methods(data_with_missing, balanced_data)
                method_results[method] = imputation_metrics
                
                filename = os.path.join(
                    self.output_dir, 
                    f"weather{int(missing_percentage * 100)}_{method}_smote.csv"
                )
...
processor = MissingDataProcessor(r'missing_data_files\weather_forecast_data.csv')
